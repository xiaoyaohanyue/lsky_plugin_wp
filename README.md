...
* WordPress.org 审核包：目录名为 `yaoyue-image-upload-for-lskypro`，入口文件为 `yaoyue-image-upload-for-lskypro.php`，符合官方插件目录 slug 习惯。

进入 WordPress 后台，在插件页面启用 `YAOYUE Image Upload for LskyPro`，然后打开 `YAOYUE Image Upload` 配置：
...
当前开发仓库建议放在测试环境的 `src/lsky_plugin_wp`，不要和 WordPress 已安装插件目录混在一起。

如果使用外层 Docker 测试环境：

* WordPress：以 `.env` 中的 `WORDPRESS_PORT` 为准，默认 `http://localhost:8080`
* phpMyAdmin：http://localhost:8081
* 已安装插件目录：`plugins/yaoyue-image-upload-for-lskypro`
* 开发源码目录：`src/lsky_plugin_wp`

常用命令：

```powershell
docker compose up -d
docker compose run --rm wpcli plugin status yaoyue-image-upload-for-lskypro
docker compose logs -f wordpress
```

如需临时测试开发源码，可从外层测试环境启用 dev override：

```powershell
docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d
docker compose run --rm wpcli plugin deactivate yaoyue-image-upload-for-lskypro
docker compose run --rm wpcli plugin activate lsky_plugin_wp
```

测试 WordPress.org 安装包时，使用默认 compose，并启用 `yaoyue-image-upload-for-lskypro`。
...
```text
build/yaoyue-image-upload-for-lskypro-wporg.zip
...
```text
yaoyue-image-upload-for-lskypro-wporg.zip
└── yaoyue-image-upload-for-lskypro/
    └── yaoyue-image-upload-for-lskypro.php
...
```json
{
  ...
  "download_url": "https://github.com/xiaoyaohanyue/lsky_plugin_wp/releases/download/v2.0.6/lsky_plugin_wp-github.zip",
  "description": "将 WordPress 媒体上传到 LskyPro（兰空图床）。",
  "changelog": "<ul><li>修复区块编辑器内容的响应式图片 srcset 地址。</li><li>新增一键修复历史图片数据工具。</li><li>补齐 Lsky 缩略图远程地址与路径元数据。</li></ul>"
}
```
...
* WordPress.org 包使用 `yaoyue-image-upload-for-lskypro/yaoyue-image-upload-for-lskypro.php` 结构
...
* 修复上传到 LskyPro 后区块编辑器前端 `srcset` 仍指向本地 `wp-content/uploads` 的问题
* 为新上传图片的缩略图 metadata 补齐远程 `url` 和 `pathname`
* 新增后台维护工具，可一键修复历史 Lsky 图片 metadata、旧 `ori_path` 和文章内容中已写入的本地 `srcset`
* 删除 Lsky 图片附件时同步清理原始本地上传文件
* 即使关闭新上传替换功能，历史 Lsky 图片仍会继续使用远程地址显示

插件名称：YAOYUE Image Upload for LskyPro
